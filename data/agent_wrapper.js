export class AgentWrapper {
    ...
    // --- 4. High-Level Job Orchestration ---
    async executeJob(jobName, options = {}) {
        this.log(`Executing Job: ${jobName}`);
        switch (jobName) {
            case 'gather_ores':
                if (options.target) {
                    return await this.gather(options.target, options.amount || 1);
                } else {
                    // Fallback looping to grab somewhat random ores
                    let amount = options.amount || 5;
                    let success = false;
                    for (let i = 0; i < amount; i++) {
                        let nearestOre = null;
                        for (let t of ['iron_ore', 'copper_ore', 'coal', 'stone']) {
                            let n = this.findNearest(t);
                            if (n) { nearestOre = t; break; }
                        }
                        if (nearestOre) {
                            success = await this.gather(nearestOre, 1);
                        } else {
                            if (this.agent && window.player) {
                                await this.agent.moveTo(window.player.x + (Math.random() * 40 - 20), window.player.y + (Math.random() * 40 - 20));
                            }
                        }
                    }
                    return success;
